...
resource "google_container_cluster" "primary" {
  ...
  resource_labels = merge({ "mesh_id" : "proj-${var.project_number}" }, var.cluster_resource_labels)
...
resource "google_container_node_pool" "pools" {
  ...
  node_config {
    ...
    machine_type     = lookup(each.value, "machine_type", "e2-standard-8")
    min_cpu_platform = lookup(var.node_pools[0], "min_cpu_platform", "")
    labels = merge(
    ...
    sandbox_config {
      content {
        sandbox_type = tobool((lookup(each.value, "enable_gke_sandbox", true))) ? ["gvisor"] : []
      }
    }
