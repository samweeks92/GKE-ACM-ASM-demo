...
resource "null_resource" "config-connector" {
  ...
  provisioner "local-exec" {
    command = "sed -i -e 's/SERVICEACCOUNTPLACEHOLDER/${google_service_account.config_connector_service_account[0].email}/g' ${path.module}/resources/config-connector.yaml && kubectl apply -f ${path.module}/resources/config-connector.yaml"
  }
